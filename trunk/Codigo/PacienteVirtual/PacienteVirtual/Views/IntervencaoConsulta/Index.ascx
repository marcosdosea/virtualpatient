        ...
        <tr>
            ...
            <% if (!(Session["_Roles"].Equals("usuario") && (Session["_IdEstadoConsulta"].Equals(3) || Session["_IdEstadoConsulta"].Equals(4) || Session["_IdEstadoConsulta"].Equals(5) || Session["_IdEstadoConsulta"].Equals(7))))
               { %>
                ...
                |
                <%: Html.ActionLink(Resources.Mensagem.remover, "Delete", "IntervencaoConsulta", new { idConsultaVariavel = item.IdConsultaVariavel, idIntervencao = item.IdIntervencao }, new { onclick = ("return confirm('Deseja realmente REMOVER esta Intervenção?')") })%>
            <% } %>
            </td>
        </tr>
        <% } %>
